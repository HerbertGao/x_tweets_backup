...
mod tests {
    ...
    fn test_config_env_var_parsing() {
        ...
        let all_true = env::var("TEST_ALL").unwrap_or_else(|_| "False".to_string()).to_lowercase() == "true";
        assert!(all_true);
        
        env::set_var("TEST_ALL", "false");
        let all_false = env::var("TEST_ALL").unwrap_or_else(|_| "False".to_string()).to_lowercase() == "true";
        assert!(!all_false);
    ...
    fn test_config_load_private_tokens_special_chars() {
        ...
        fs::write(&token_file, "KEY1=value with\nnewline\nKEY2=value with=equals").unwrap();
